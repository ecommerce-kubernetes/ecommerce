...
> 자세한 설명은 [Buynest Docs](https://api.buynest.cloud) 에 접속해 확인하실 수 있습니다.
...
<p align="center">
  <img width="771" height="792" alt="image" src="https://github.com/user-attachments/assets/1c92208b-4b03-40a1-9510-a2ec8b107dc7" />
</div>
...
<p align="center">
  <img width="931" height="572" alt="image" src="https://github.com/user-attachments/assets/1aa5520b-5978-43a8-b5a9-b071efa005f2" />
</div>
...
<table width="100%">
  <thead>
    <tr>
      <th align="left">서비스 (Service)</th>
      <th align="left">핵심 역할 (Core Domain)</th>
      <th align="left">구현 상세</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      ...
      <td>라우팅/인증</td>
      <td>
        • <strong>Spring Cloud Gateway</strong>: Netty 기반의 Non-blocking I/O 아키텍처<br/>
        • <strong>JWT Filter</strong>: 인증 필터를 통한 토큰 검증 및 파싱<br/>
        • <strong>Load Balancing</strong>: 라운드 로빈 방식의 서비스 라우팅
      </td>
    </tr>
    <tr>
      <td><strong>Discovery Service (Eureka)</strong></td>
      <td>서비스 등록</td>
      <td>
        • <strong>Netflix Eureka</strong>: 마이크로 서비스 인스턴스 상태(Up/Down) 감지<br/>
        • <strong>Heartbeat</strong>: 주기적인 헬스 체크를 통해 가용성 없는 인스턴스 자동 제거
      </td>
    </tr>
    <tr>
      <td><strong>Config Service</strong></td>
      <td>설정 중앙화</td>
      <td>
        • <strong>Spring Cloud Config</strong>: Git 저장소 기반 설정 이력 관리<br/>
        • <strong>Cipher Encryption</strong>: DB 비밀번호 등 민감 정보 암호화 (<code>{cipher}...</code>)
      </td>
    </tr>
    <tr>
      ...
      <td>회원 관련</td>
      <td>
        • <strong>Spring Security</strong>: JWT 기반 인증 아키텍처<br/>
        • <strong>Redis</strong>: Refresh Token 저장소
      </td>
    </tr>
    <tr>
      ...
      <td>상품 관련</td>
      <td>
        • <strong>Spring Security</strong>: 커스텀 헤더 기반 인증<br/>
        • 상품 엔티티 저장/조회/수정
      </td>
    </tr>
    <tr>
      ...
      <td>주문 관련</td>
      <td>
        • <strong>Event-Driven</strong>: Kafka 기반 비동기 주문 생성<br/>
        • <strong>SAGA Pattern</strong>: 분산 트랜잭션 데이터 정합성 보장 (포인트/재고 보상 트랜잭션)
      </td>
    </tr>
  </tbody>
</table>
