...
public class UsePointCommandRouter implements PointSagaCommandRouter {
    private final PointSagaProcessor processor;

    @Override
    public boolean supports(PointSagaCommand command) {
        return command == PointSagaCommand.USE_POINT;
    }

    @Override
    public void execute(Long sagaId, PointSagaCommandPayload payload) {
        processor.deduct(sagaId, payload.executionId(), payload.userId(), Money.wons(payload.usedPoints()));
    }

    @Override
    public void fail(Long sagaId, PointSagaCommandPayload payload, String errorCode) {
        processor.failDeduct(sagaId, payload.executionId(), errorCode);
    }
}
