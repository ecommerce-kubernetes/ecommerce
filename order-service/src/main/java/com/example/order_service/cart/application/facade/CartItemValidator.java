...
public class CartItemValidator {
    ...
    public void validate(AddCartItemsCommand command, CartProductResult productData) {
        ...
        for (AddCartItemsCommand.Item item : command.items()) {
            CartProductResult.CartProductDetail product = productDataMap.get(item.productVariantId());
            if (product == null) {
