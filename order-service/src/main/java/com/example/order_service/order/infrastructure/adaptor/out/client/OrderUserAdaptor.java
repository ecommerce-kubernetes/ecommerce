...
import com.example.order_service.order.infrastructure.adaptor.out.client.mapper.OrderUserAdaptorMapper;
...
public class OrderUserAdaptor implements OrderUserPort {
    ...
    private final OrderUserAdaptorMapper orderUserAdaptorMapper;

    @Override
    public OrdererProfileResult getOrdererProfile(Long userId) {
        UserProfileResponse response = executeGetUserProfile(userId);
        return orderUserAdaptorMapper.mapToOrdererProfileResult(response);
    }
    ...
    public OrdererPointResult getOrdererPoints(Long userId) {
        try {
            ...
            return orderUserAdaptorMapper.mapToOrdererPointResult(response);
