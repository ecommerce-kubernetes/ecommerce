...
- 주문 항목 최종 금액을 계산할때 주문항목 최종 금액은 0원 이상이여야 한다.
- 주문 항목의 최종 금액을 계산할 때 적용된 상품 쿠폰 할인 금액이 판매가 총액을 초과하는 경우, 실제 적용되는 쿠폰 할인 금액은 판매가 총액을 최대 한도로 적용된다.
-
...
- 할인 금액이 1원단위라면 10원 단위로 절삭된다.

### 4-4. 정률 할인 쿠폰 정책(RateCouponDiscountPolicy)
