...
public class OrderDescriptor {
    ...
    public static FieldDescriptor[] orderResponse() {
        return new FieldDescriptor[] {
                ...
                fieldWithPath("orderItems[].itemPayment.lineTotal").type(JsonFieldType.NUMBER).description("주문 항목 상품 판매가 총액"),
                fieldWithPath("orderItems[].itemPayment.couponDiscount").type(JsonFieldType.NUMBER).description("상품 쿠폰 할인 금액"),
                fieldWithPath("orderItems[].itemPayment.finalItemAmount").type(JsonFieldType.NUMBER).description("최종 주문 항목 가격 총액"),
                fieldWithPath("paymentSummary.totalOriginalAmount").type(JsonFieldType.NUMBER).description("총 상품 금액"),
                fieldWithPath("paymentSummary.totalItemDiscount").type(JsonFieldType.NUMBER).description("총 상품 할인 금액"),
                fieldWithPath("paymentSummary.totalItemCouponDiscount").type(JsonFieldType.NUMBER).description("총 상품 쿠폰 할인 금액"),
                fieldWithPath("paymentSummary.cartCouponDiscount").type(JsonFieldType.NUMBER).description("장바구니 쿠폰 할인 금액"),
                fieldWithPath("paymentSummary.usedPoints").type(JsonFieldType.NUMBER).description("사용 포인트"),
                fieldWithPath("paymentSummary.totalPaymentAmount").type(JsonFieldType.NUMBER).description("총 결제 금액"),
