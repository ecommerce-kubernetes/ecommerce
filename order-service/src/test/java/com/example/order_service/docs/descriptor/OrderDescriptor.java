...
public class OrderDescriptor {
    ...
    public static FieldDescriptor[] orderSummaryResponse() {
        return new FieldDescriptor[] {
                ...
                fieldWithPath("content[].orderItems[].orderItemAmount.originalAmount").type(JsonFieldType.NUMBER).description("주문 항목 원 가격"),
                fieldWithPath("content[].orderItems[].orderItemAmount.itemDiscount").type(JsonFieldType.NUMBER).description("주문 항목 상품 할인 가격"),
                fieldWithPath("content[].orderItems[].orderItemAmount.lineTotal").type(JsonFieldType.NUMBER).description("주문 항목 상품 판매가 총액"),
                fieldWithPath("content[].orderItems[].orderItemAmount.couponDiscount").type(JsonFieldType.NUMBER).description("상품 쿠폰 할인 금액"),
                fieldWithPath("content[].orderItems[].orderItemAmount.finalItemAmount").type(JsonFieldType.NUMBER).description("최종 주문 항목 가격 총액"),
