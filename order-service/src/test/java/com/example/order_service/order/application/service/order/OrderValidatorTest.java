...
public class OrderValidatorTest {
    ...
    void validateOrderProduct_missing_product() {
        ...
        OrderProductsResult.OrderProductDetail info = OrderProductsResult.OrderProductDetail.builder()
                ...
                .build();
        OrderProductsResult productList = OrderProductsResult.builder()
    ...
    void validateOrderProduct_unorderable_product() {
        //given
        OrderSheet orderSheet = createOrderSheet();
        List<OrderProductsResult.OrderProductDetail> infos = orderSheet.getItems().stream()
                .map(item -> OrderProductsResult.OrderProductDetail.builder()
                        .productSnapshot(item.getProductSnapshot())
    ...
    void validateOrderCartCoupon() {
        //given
        OrderSheet orderSheet = createOrderSheet();
        OrderProductsResult productResult = createValidProductList(orderSheet);
...
//                .itemCoupons(invalidItemCoupons)
    ...
    void validateOrderPoints() {
        //given
        OrderSheet orderSheet = createOrderSheet();
        OrderProductsResult productList = createValidProductList(orderSheet);
    ...
    private OrderProductsResult createValidProductList(OrderSheet orderSheet) {
        List<OrderProductsResult.OrderProductDetail> infos = orderSheet.getItems().stream().map(item ->
                        OrderProductsResult.OrderProductDetail.builder()
                                .productSnapshot(item.getProductSnapshot())
                                .status(OrderProductStatus.ON_SALE)
                                .stock(item.getQuantity() + 100)
                                .priceSnapshot(item.getPriceSnapshot())
